function cost = cost_function(pose,pose_gt,perception_bag_i,MAP,Inter_Para_real,i_step)
...
for i=1:1:size(perception_bag_i{2},1)
    ...
    if length(uv_lane) >1
        ...
        if (~ifreasonable_lane_cost(uv_lane(1:2),uv_lane(3:4),perception_bag_i{2}(i,1:4)))
            dis_1 = 100;
            dis_1 = 100;
            aa=1;
        end
